...
该模块实现了使用VGG16（带BatchNorm）作为编码器（backbone）的U-Net分割网络。
VGG16是一个经典的深度卷积神经网络，具有强大的特征提取能力。

主要组件：
- IntermediateLayerGetter: 从backbone中提取中间层特征的包装器
- VGG16UNet: 完整的VGG16-U-Net分割模型

优势：
- 特征提取能力强：VGG16在ImageNet上预训练，具有丰富的语义特征
- 精度高：适合对精度要求较高的分割任务
- 稳定性好：使用BatchNorm提高训练稳定性
"""
...
from torchvision.models import vgg16_bn  # VGG16（带BatchNorm）预训练模型
...
class VGG16UNet(nn.Module):
    ...
    该模型使用VGG16（带BatchNorm）作为编码器（backbone），提取多尺度特征，
    然后通过U-Net的解码器结构进行上采样，最终输出分割结果。
    
    网络结构：
    1. 编码器（VGG16）：提取5个不同尺度的特征图
       - stage0: 64通道（浅层特征，细节丰富）
       - stage1: 128通道
       - stage2: 256通道
       - stage3: 512通道
       - stage4: 512通道（深层特征，语义丰富）
    ...
    def __init__(self, num_classes, pretrain_backbone: bool = False):
        ...
        Args:
            ...
            pretrain_backbone (bool): 是否使用ImageNet预训练的VGG16权重
        ...
        #     # 载入vgg16_bn预训练权重
        #     # 下载地址：https://download.pytorch.org/models/vgg16_bn-6c64b313.pth
        #     backbone.load_state_dict(torch.load("vgg16_bn.pth", map_location='cpu'))

        # 只使用VGG16的features部分（去掉分类头）
        backbone = backbone.features

        # ==================== 2. 定义要提取的特征层 ====================
        # stage_indices: 指定要提取的层索引
        # VGG16的features包含43个模块（包括卷积层、BatchNorm层、ReLU层、MaxPool层）
        # 这里选择5个关键层用于跳跃连接，对应不同的特征尺度
        # 索引对应：浅层（细节）-> 深层（语义）
        # 5: 第一个MaxPool后的特征（64通道）
        # 12: 第二个MaxPool后的特征（128通道）
        # 22: 第三个MaxPool后的特征（256通道）
        # 32: 第四个MaxPool后的特征（512通道）
        # 42: 最后一个MaxPool后的特征（512通道）
        stage_indices = [5, 12, 22, 32, 42]
        
        # 定义每个阶段的输出通道数
        # VGG16的特征通道数：64 -> 128 -> 256 -> 512 -> 512
        self.stage_out_channels = [64, 128, 256, 512, 512]
        
        # 创建return_layers字典，将层索引映射到stage名称
        # 例如：{5: "stage0", 12: "stage1", 22: "stage2", 32: "stage3", 42: "stage4"}
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # backbone_out包含：
        # - 'stage0': 最浅层特征（64通道，细节丰富，分辨率高）
        # - 'stage1': 较浅层特征（128通道）
        # - 'stage2': 中间层特征（256通道）
        # - 'stage3': 较深层特征（512通道）
        # - 'stage4': 最深层特征（512通道，语义丰富，分辨率低）
